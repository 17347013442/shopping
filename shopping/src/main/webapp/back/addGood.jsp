...
<body>
<div>
	<form role="form"   novalidate id="form" enctype="multipart/form-data">
    ...
    <div class="invalid-feedback"><small>tino不能为空</small></div><!-- 验证失败出现 -->
	</div>
  <div class="form-group col-sm-4">
    <label for="name">商品名称:</label>
    <input type="text" class="form-control" id="name" name="gname" placeholder="请输入商品名称">
      <div class="invalid-feedback"><small>商品名称不能为空</small></div><!-- 验证失败出现 -->
    ...
    <input type="file" id="pics" name="upload1" >
...
{ 
	...
	//console.log('tno'+tno);
	//根据tno查找所有的tino
	$.post("../type/findByTno",{tno:tno},function(data){
		//console.log(data);
...
$(function(){
	...
	//查询所有的tno
$.post("../type/findType",function(data){
	//console.log(data);
	var str='';
	for(var i=0;i<data.length;i++){
...
//action="../good/addGood" method="post"
...
$('#btn1').click(function(){
	...
	var unit=$('#unit').val();
	var place=$('#place').val();
	var qualityPeriod=$('#qualityPeriod').val();
	var upload1=$('#pics').val();
	var descr=$('#editor_id').val();
	var inprice=$('#inprice').val();
	
	//alert(tino);
	var formData = new FormData($("#form")[0]);
	//formData.append("gname",gname);
    //formData.append("tino",tino);
	...
	//}
	// $('#form').addClass("was-validated")//bootstrap提供给验证失败后出现的
	//
});
